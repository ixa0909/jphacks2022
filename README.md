...
この偏った食事を解決する既存の方法の多くは家庭での料理です。しかし、料理はハードルが高い、する時間がない人もいます。
そこで、外食での偏ったメニューの注文の解消を目的とした Web アプリケーションの開発をしました。
...
### なぜ問題を解決できるのか？
偏った食事の問題に外食料理店のメニュー制覇というゲーム性・視認性を与えることで食への興味を高め、様々な料理を食べてもらうことができるから。
また、より外食を楽しむことができることに加えて、コロナ禍が落ち着いてきたこの時期に合わせて長らく続いた外食が増える時期だからこそです。
...
### 特に力を入れて作成したコード
#### 1. [データベース内のテーブルへのデータ挿入コード](https://github.com/jphacks/D_2212/tree/backend/backend/insert_to_table)
理由: 繰り返し操作はコード化することで効率化しました。
#### 2. [Web スクレイピング](https://github.com/jphacks/D_2212/blob/backend/backend/get_image_gyuudon.py)
